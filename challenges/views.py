...
def flagsubmit(request) :
	...
	names=models.Challenges.objects.all()
	searchname=names[1].name

	# print(searchname.lower(),flag_submit_id)
	# if flag_submit_id == searchname.lower():
	# 	print(searchname)
	# 	print("correct flag is " +" "+names[1].flag)
	# else: 
	# 	print("Not accable")
	 
	# flag=names[1].flag
	# points=names[1].points


	for i in range(0,20):
		searchname=names[i].name

		if flag_submit_id == searchname.lower():
			print(searchname)
			print("correct flag is " +" "+names[i].flag)
			flag=names[i].flag
			points=names[i].points
			break
		else: 
			continue
	
	# flag = models.Challenges.objects.get(name=flag_submit_id).flag
	# points = models.Challenges.objects.get(name=flag_submit_id).points
